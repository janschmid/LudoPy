"""Run Q-learning main function, this function implements high level methods such as startTesting, startLearning.
Results are saved in the folder Savegames and TestResults"""
import os.path as p
import sys

script_dir = p.dirname(p.realpath(__file__))
sys.path.insert(1, p.join(p.dirname(script_dir), "YARAL", "src"))
import ludopy
import matplotlib.pyplot as plt
import numpy as np
import progressbar
from stateSpacePlayer import StateSpacePlayer


def startTesting(learningRate, discountFactor, iterations, players, oldVersion=False, againstQPlayer=True):
    """Start a single test, result will be saved in TestResults """
    from Qlearn import Qplayer

    resultName = "OV_{4}_{0}_pl{3}_df_{1}_lr{2}_againstQPlayer_{5}.csv".format(
        "finalGameAgainstYARAL", discountFactor, learningRate, players, oldVersion, againstQPlayer
    )
    ...
    np.savetxt(
        ...
        delimiter=",",
        fmt="%1.9f",
        header="idx, player1WinningAvg, qTableChange, player2WinningAvg",
    )


def startLearning(learningRate, discountFactor, iterations, players, oldVersion=False):
    """Start leanring, results are stored in a csv file to be loaded after learning for testing"""
    friendlyName = "test"
    if players not in {2, 4}:
        raise ValueError("results: players must 2 or 4")
    ...
    np.savetxt(
        "TestResults/" + name, resultArray, delimiter=",", fmt="%1.9f", header="idx, player1WinningAvg, qTableChange"
    )
...
def testWithLearningRate(learningRate):
    """Start leaning with a specified learning rate"""
...
def startParallelTesting():
    ...
    proc = []
    proc.append(Process(target=startTesting, args=(0.06, 0.2, 2, 2, False, False)))
    for p in proc:
        p.start()
    for p in proc:
        p.join()
...
if __name__ == "__main__":
    ...
    startLearning(0.06, 0.2, 500, 2, False)
    startTesting(0.06, 0.2, 500, 2, False, False)
