...
class StateSpace:
        ...
        for player in players:
            ...
            for piece in player.pieces:
                if piece == 0:
                    ...
                    self.GlobalPlayerPosition[indexPlayer].pieces[indexPiece] = (
                        piece + (self._quarterGameSize * indexPlayer)
                    ) % 52
    ...
    def _UpdateKillAction(self, player, piece, dice, killList):
        ...
        if (
            targetPos in killList
            and targetPos not in self._globePositionsGlobal
            and localTargetPos not in self._globePositionsLocal
            and localTargetPos not in self._dangerPositionsLocal
        ):
            nextState = self._GetTargetPlayerState(player, piece, dice).value
            self.UpdateActionTable(player, Action(Action.H_Kill.value + nextState * 10), piece, 1)
